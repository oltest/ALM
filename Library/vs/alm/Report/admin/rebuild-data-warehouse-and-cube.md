Title: Rebuild the data warehouse and cube | Team Foundation Server 
Description: Rebuild the data warehouse and cube when connecting to an on-premises Team Foundation Server (TFS) 
ms.TocTitle: Rebuild the data warehouse and cube
ms.ContentId: 23CD5F6B-468D-47B5-8A03-96547B526C2D
...
For more information, see [Add accounts to administer TFS](https://msdn.microsoft.com/library/bb552341.aspx).
...
1. Log on to the appropriate server, open Computer Manager, and then verify that the services and application pools in the following table are running: 
	...
	<td><ul><li>SQL Server Analysis Services (MSSQLSERVER or <i>TFSInstance</i>).</li></ul></td>
	</tr>
	<tr>
	<td>Team Foundation databases</td>
	<td><ul><li>SQL Server (MSSQLSERVER or <i>TFSInstance</i>).</li><li>SQL Server Agent (MSSQLSERVER or <i>TFSInstance</i>).</li></ul></td>
	</tr>
	<tr>
	<td>Application tier</td>
	<td><ul><li>Microsoft Team Foundation Server Application Pool.</li></ul></td>
	</tr>
	</table>

	For more information, see [Stop and start services, application pools, and websites](https://msdn.microsoft.com/library/cc668752.aspx).
...
1. [Open the Team Foundation administration console](https://msdn.microsoft.com/library/dd273719%28v=vs.120%29.aspx).
...
4. Choose **OK** to initiate the rebuild process.  
	...
	* Processes the cube according to its regular processing interval.  
	...
	<blockquote style="font-size: 13px">**Note:  **While the warehouse and the cube are being rebuilt, the administration console for Team Foundation may stop responding. You might need to refresh the administration console after the warehouse and the cube have been rebuilt.</blockquote>  
...
* You manually modified a TFS database or edited a SQL table. You should not manually modify any of the TFS databases unless you’re either instructed to do so by Microsoft Support or when you’re following the procedures described for manually backing up the databases ([Manually back up Team Foundation Server](https://msdn.microsoft.com/library/ms253070.aspx)). Any other modifications can invalidate your service agreement, block upgrades and patches, and result in data loss or corruption.  
...
* You’ve performed an unsupported backup or restore operation as described in [Back up and restore TFS](https://msdn.microsoft.com/library/bb552295.aspx).   
