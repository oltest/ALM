Title: Configure features after an upgrade to the application-tier server | Team Foundation Server 
Description: Configure features after an upgrade to  the application-tier server of Team Foundation Server (TFS)
ms.TocTitle: Configure features after an upgrade
ms.ContentId: A71B6DD4-2780-4B61-B863-03BBE44AC953
...
<blockquote style="font-size: 13px"><b>Note:  </b>If you're upgrading from TFS 2010 or an earlier version, review the steps outlined [here](before-you-upgrade-tfs.md). <br/>
If you need to upgrade TFS, [go to the downloads page](https://www.visualstudio.com/downloads/visual-studio-2015-downloads-vs.aspx). <br/> 
If you have several team projects to update, you can [apply updates programmatically](#program-updates). </blockquote>  
...
1. From the web portal home page, open the administration context.  
	...
	If you're not a team project administrator, [get added as one](https://msdn.microsoft.com/library/bb558971.aspx). Only project administrators or TFS administrators can run the wizard.
...
3. If you see a **Configure Features link**, run the wizard. 
	...
	If Test Plan and Test Suite are not configured, you'll need to [manually import the WIT definitions and migrate data](https://msdn.microsoft.com/library/dn789837.aspx).  
...
*	If your team project is based on a process template other than those that TFS provides, [update the customized process template used to create the team project](update-customized-process-template.md), and then run the Configure Features wizard. 
	...
	TFS process templates correspond to Scrum, Agile, and CMMI  as described [here](../guidance/choose-process.md).
...
* If your team project was based on a v4.2 or earlier process template version, first [apply the updates to comply with TFS 2012 manually](https://msdn.microsoft.com/library/ff452591.aspx). Then run the Configure Features wizard.
...
- To view and work with Epic and Feature portfolio backlogs, you may need to [activate them for the team](../backlogs/organize-backlog.md)  
- To use Code Review and Feedback Request, you need to [configure an SMTP server for TFS](https://msdn.microsoft.com/library/ms400808.aspx).  
...
- [Before you upgrade TFS](before-you-upgrade-tfs.md)
...
If you have additional questions, you can post one or search for answers in the [Team Foundation Server - Work Item Tracking](http://social.msdn.microsoft.com/Forums/tfsworkitemtracking/threads) forum.


<a id="program-updates"> </a>
###Programmatically update many team projects 
...
See  [Manual updates to support test plans and test suites](https://msdn.microsoft.com/library/dn789837.aspx).
...
3. For each feature that can be configured, uploads the definitions for new objects or modifies existing definitions. To learn how the wizard configures each specific feature, see [Configuration actions made to add each feature](https://msdn.microsoft.com/library/hh913785.aspx).
