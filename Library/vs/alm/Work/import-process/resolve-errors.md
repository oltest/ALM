Title: Resolve validation errors | Visual Studio Online  
Description: Fix errors reported upon importing a process to support customization of tracking work in Visual Studio Online.  
ms.TocTitle: Resolve validation errors  
ms.ContentId: 2407FB2B-FAE6-4BBB-99CB-B88904293A43  
...
<td>
<ul>
<li>[Information messages](#info-only)</li>
<li>[Miscellaneous file errors](#file-errors) </li>
<li>[Process template plug-in errors](#process-templates) </li>
<li>[Classification plug-in errors](#classification) </li>
<li>[Test Management plug-in errors](#test-management) </li>
<li>[Categories](#categories) </li>
</ul>
</td>
<td>
<ul>
<li>[Link types](#link-types)</li>
<li>[Global lists](#global-lists) </li>
<li>[Process configuration](#process-configuration)</li>
<li>[Work item type](#wit-definitions) </li>
<li>[Work item fields](#wit-fields)</li>
<li>[Workflow](#wit-workflow-definitions)</li></ul>
</td>
...
#### Scenerio example
1. [Create](https://msdn.microsoft.com/library/ms404855.aspx) new work item type called "LSI"  
``` 
...
The ProcessTemplate.xml definition file must conform to the syntax and rules described in [ProcessTemplate XML element reference](http://msdn.microsoft.com/library/aa395261.aspx).   
...
Friendly names must be unique across all field definitions for all WIT definitions specified for all processes imported to Visual Studio Online.
...
<a id="categories"></a>
## Categories
The Categories.xml definition file must conform to the syntax and rules described in [Categories XML element reference](http://msdn.microsoft.com/library/dd469527%28v=vs.140%29.aspx).     
...
```
Reference topics: 
*   [ProcessConfiguration XML element reference](http://msdn.microsoft.com/library/hh500408%28v=vs.140%29.aspx)
*   [Categories XML element reference](http://msdn.microsoft.com/library/dd469527%28v=vs.140%29.aspx)


<a id="classification"></a>
## Classification plug-in errors
The Classification.xml definition file must conform to the syntax and rules described in [Classification plug-in](http://msdn.microsoft.com/library/ms243840.aspx).     
...
Reference: [Add and modify area and iteration paths](http://go.microsoft.com/fwlink/?LinkId=543701).
...
For reference syntax, see [Define pick lists](http://msdn.microsoft.com/library/ms194947.aspx).  
...
The ProcessConfiguration.xml definition file must conform to the syntax and rules described in [ProcessConfiguration XML element reference](http://msdn.microsoft.com/library/hh500408%28v=vs.140%29.aspx).   
...
Review [ProcessConfiguration XML element reference](http://msdn.microsoft.com/library/hh500408%28v=vs.140%29.aspx) for required elements.
...
Review [ProcessConfiguration XML element reference](http://msdn.microsoft.com/library/hh500408%28v=vs.140%29.aspx) for required ```TypeField``` elements. 
#### Example
...
Review [ProcessConfiguration XML element reference](http://msdn.microsoft.com/library/hh500408%28v=vs.140%29.aspx) for more information.
...
The namespaces-System.*XXX* and Microsoft.VSTS.*XXX*-are reserved by Visual Studio Online. 

See [All WITD XML elements reference](https://msdn.microsoft.com/library/ms243932.aspx) for more information.
...
The following rules are **not** supported for import to Visual Studio Online: 
...
Global lists are not supported in Visual Studio Online. 
Replace all instances of ```GLOBALLIST``` elements with 
```ALLOWEDVALUES``` and ```LISTITEM``` elements in all WIT definition files.
Reference:  [Define pick lists](https://msdn.microsoft.com/library/ms194947.aspx).
...
Global lists are not supported in Visual Studio Online. Replace all instances of ```GLOBALLIST``` elements with 
```ALLOWEDVALUES``` and ```LISTITEM``` elements in all WIT definition files.
Reference: [Define pick lists](https://msdn.microsoft.com/library/ms194947.aspx).
...
The namespaces-System.*XXX* and Microsoft.VSTS.*XXX*-are reserved in Visual Studio Online. 
...
Fields must be defined consistently across processes that have been or that you plan to import to Visual Studio Online. 
...
have been imported or that you plan to import to Visual Studio Online.
...
<a id="TF402568"></a>
### TF402568: You've defined *[N]* fields with ```syncnamechanges="true"``` for work item type *[WITNAME]*. A maximum of *[syncnameLIMIT]*  \is allowed.
...
The ```"for"``` and ```"not"``` attributes are not supported at all for any field rule for import to Visual Studio Online.  
...
All fields defined by Visual Studio Online in the reserved namespaces-System.*XXX* and Microsoft.VSTS.*XXX*-must not be altered.  If you wish to have a different list of values in these picklist fields, define your own custom field.
...
that have been imported or will be imported to Visual Studio Online. 
...
Custom controls are not supported in Visual Studio Online. Review the ```FORM``` section for the named WIT and remove any custom controls that are defined. 
...
must conform to the syntax and rules described in [Define the initial configuration of Test Manager](http://msdn.microsoft.com/library/dd997878.aspx). 
